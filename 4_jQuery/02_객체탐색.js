...
// 홀수번째 인덱스 요소를 가져와서 배경색 핫핑크 글자색 화이트
$("h4").filter(":odd").css("background-color", "hotpink").css("color", "white");

// not(선택자) : 선택된 요소들 중 특정 요소와 일치하지 않는 요소 선택
$("h4:not(.test").css({ backgroundColor: "orange", color: "white" });

// 2. Ancestors & Descendants 메서드
// parent : 선택된 요소의 바로 위 상위 요소 하나 선택
// span 요소의 부모 요소에 글자색 : red, 테두리 : 2ps solid
$("span").parent().css({ color: "red", border: "2px solid" });

// parents('선택자') : 선택된 요소의 모든 상위 요소를 선택하거나,
// 매개변수가 있으면 매개 변수와 일치하는 상위 요소만 선택
// li 요소의 모든 상위 요소에 글자색 : bule
$("li").parents().css({ color: "blue" });
$("li").parents("div").css({ border: "2px dashed tomato" });

// parentsUntil('선택자') : 선택된 요소의 모든 상위 요소를 선택하거나,
// 매개변수가 있으면 선택된 요소부터 매개변수와 일치하는 요소까지의 상위 요소선택
// span 요소부터 div요소 이전까지 상위 요소에 배경색 : lavenderblush
...
// find(선택자) : 선택된 요소의 하위 요소중 매개변수와 일치하는 모든 하위 요소
